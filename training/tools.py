    ...
    def get_train_dl(self,force=False):
        if force: #force will regenerate the dls - new random dist or changing split
            ...
            self.train_dl = DataLoader(self.train_set, batch_size=self.batch_size_train,
                        drop_last=True, shuffle=True, num_workers=self.num_workers, pin_memory=True)
        else:
            if self.train_set is None:
                ...
                if self.single_split:
                    ...
                    self.train_dl = DataLoader(self.full_train_set, batch_size=self.
                            batch_size_train,drop_last=True, shuffle=True, num_workers=self.num_workers, pin_memory=True)
    ...
    def _load_sets(self):
        ...
        if self.normalize:
            tf = transforms.Compose([
                transforms.ToTensor(),
                transforms.Normalize(mean=[0.13066048], std=[0.30810780])
            ])
        else:
            tf = transforms.Compose([
                transforms.ToTensor()
        ...
        self.full_train_set = torchvision.datasets.MNIST('./data/mnist',
                download=True, train=True, transform=tf)
        #full testing set
        self.full_test_set = torchvision.datasets.MNIST('./data/mnist',
                download=True, train=False, transform=tf)
...
class IMAGENETDataColl(DataColl):
    def _load_sets(self):
        #child version of function, IMAGENET specific
        if self.normalize:
        ...
        #full training set
        self.full_train_set = torchvision.datasets.ImageFolder(
                './data/imagenet/ILSVRC/Data/CLS-LOC/train', tf_train)
        #full testing set
        self.full_test_set = torchvision.datasets.ImageFolder(
                './data/imagenet/ILSVRC/Data/CLS-LOC/val_as_test/', tf_test)

    def get_train_dl(self):
        if self.train_dl is None:
            self.train_dl = DataLoader(self.full_train_set, batch_size=self.batch_size_train,
                    shuffle=True, num_workers=self.num_workers, pin_memory=True)
        return self.train_dl

    def get_valid_dl(self):
        if self.valid_dl is None:
            self.valid_dl = DataLoader(self.full_test_set, batch_size=self.batch_size_train,
